...
const StateTable = ({ 
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (editingState) {
      onEditState(editingState.id, formData);
    } else {
      onAddState(formData);
    }
    setIsModalOpen(false);
    setFormData({ name: '', code: '', countryId: selectedCountry || '' });
            ...
            {filteredStates.length > 0 ? filteredStates.map((state) => (
              <tr key={state.id} className="hover:bg-slate-50">
                ...
                <td className="p-3 text-slate-600">
                  {countries.find(c => c.id === state.countryId)?.name || 'Unknown'}
                </td>
        ...
        <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md border border-slate-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">State Code *</label>
                <input
                  type="text"
                  value={formData.code}
