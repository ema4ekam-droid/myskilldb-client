  ...
  return (
    <section className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-full">
      <h2 className="text-xl font-bold mb-4 text-slate-900">
        {selectedState ? `Educational Boards in ${selectedState}` : 'Educational Boards'}
      </h2>
      
      {selectedState ? (
        <div>
          <form onSubmit={onAddBoard} className="flex gap-2 mb-4">
            <input
              type="text"
              className="w-full bg-slate-100 border-slate-200 rounded-md p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
              placeholder="New Board Name (e.g., CBSE, IB, ICSE)"
              value={newBoardName}
          ...
          <div className="space-y-2 h-[52vh] overflow-y-auto">
            {isLoadingBoards ? (
              <p className="text-slate-500 text-center">Loading boards...</p>
            ) : boards.length === 0 ? (
              <p className="text-slate-400 text-center">No boards found for {selectedState}.</p>
            ) : (
              boards.sort().map((board) => (
                <div key={board} className="flex justify-between items-center bg-slate-100 p-3 rounded-md">
        ...
        <p className="text-slate-400 text-center mt-8">Select a state to view and manage its educational boards.</p>
      )}
    </section>
  );
};
