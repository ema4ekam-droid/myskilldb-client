...
const OrgMenuNavigation = ({ currentPage, onPageChange }) => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      color: 'green',
      subItems: [
        { id: 'view-classrooms', label: 'View Classrooms', icon: 'fas fa-list' },
        { id: 'define-subjects', label: 'Define Subjects', icon: 'fas fa-book' },
    ...
    },
    {
      ...
      color: 'purple',
      subItems: [
        { id: 'topic-management', label: 'Topic Management', icon: 'fas fa-tags' },
        { id: 'test-management', label: 'Test Management', icon: 'fas fa-clipboard-check' },
    ...
    },
    {
      ...
      color: 'orange',
      subItems: [
        { id: 'school-reports', label: 'School Reports', icon: 'fas fa-file-alt' },
        { id: 'student-ratings', label: 'Student Ratings', icon: 'fas fa-star' }
      ]
    },
    {
      id: 'student-profiles',
  ...
  React.useEffect(() => {
    const activeSubmenuParent = menuItems.find(item => 
      item.subItems && item.subItems.some(subItem => subItem.id === currentPage)
    );
      ...
      `}>
        ...
        <div className="p-6 border-b border-slate-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">Organization Admin Panel</p>
            </div>
        ...
        <nav className="p-4 space-y-1">
          {menuItems.map((item) => {
            ...
            const hasActiveSubmenu = item.subItems && item.subItems.some(subItem => currentPage === subItem.id);
            ...
            return (
              <div key={item.id}>
                ...
                >
                  ...
                  {item.subItems && (
                    <i className={`fas fa-chevron-${isExpanded ? 'up' : 'down'} text-xs transition-transform duration-200 ${
                      hasActiveSubmenu ? 'text-indigo-500' : isExpanded ? 'text-slate-500' : 'text-slate-400'
                    }`}></i>
                  )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-200 bg-slate-50">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-900 truncate">Organization Admin</p>
