      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Assign sections to classes under specific departments
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              onFilterChange((prev) => ({ ...prev, classId: e.target.value }))
            }
            className={inputBaseClass}
          >
          ...
          <div className="flex flex-wrap gap-2">
            {appliedFilters.departmentId && (
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                Department:{" "}
                {
                  departments.find((d) => d._id === appliedFilters.departmentId)
                    ?.name
                }
              </span>
            )}
            {appliedFilters.classId && (
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                Class:{" "}
                {classes.find((c) => c._id === appliedFilters.classId)?.name}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          {Object.keys(groupedAssignments).length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              ...
              <p>
                No assignments found. Create your first assignment or adjust
              ...
              >
                    ...
                    <div className="flex items-center gap-3">
                      <i
                        className={`fas fa-chevron-${
                          expandedDepartments[deptId] ? "down" : "right"
                ...
                {expandedDepartments[deptId] && (
                  <div className="divide-y divide-slate-100">
                    {Object.entries(deptData.classes).map(
                      ([classId, classData]) => (
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                            {classData.sections.map((section) => (
                              ...
                              >
                                ...
                                <button
                                  onClick={() =>
                                    onDeleteAssignment(section.assignmentData)
                                  }
